...
dimGradSolver = 100#25#100#
rateDropout = 0.2
DimAE = 32#10
flagAEType = 'unet2'#u'unet-1d-tanh'#'unet2'#unet2'#'unet-1d-tanh'#'unet-1d-relu'#'#ode'#' 'ode'#'unet'#'unet2+wc_ode'#'unet' # #'ode' # 
#flagAEType = 'unet-1d'
...
elif flagAEType == 'unet2-1d-tanh': ## Conv model with no use of the central point        
    class Phi_r(torch.nn.Module):
      def __init__(self):
          super(Phi_r, self).__init__()
          self.nfeat = DimAE
          self.unet  = unet_1d.UNet_1D_5scales(3,3,False,self.nfeat,activation='tanh')
          
      def forward(self, xinp):
          xout = self.unet( xinp.view(-1,xinp.size(1),xinp.size(2)) )
          return xout.view(-1,xinp.size(1),xinp.size(2),1)

elif flagAEType == 'unet-1d-bilin': ## Conv model with no use of the central point        
    class Phi_r(torch.nn.Module):
      def __init__(self):
          super(Phi_r, self).__init__()
          self.nfeat = DimAE
