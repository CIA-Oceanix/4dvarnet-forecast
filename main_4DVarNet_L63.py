...
rateDropout = 0.2
DimAE = 10
flagAEType = 'unet-1d-8'#'unet2'#ode'#'unet2'# 'ode'#'unet'#'unet2+wc_ode'#'unet' # #'ode' # 
#flagAEType = 'unet-1d'
  ...
  class Phi_r(torch.nn.Module):
    ...
    class UNet_1D(torch.nn.Module):
        def __init__(self, n_channels, n_classes, bilinear=False):
            ...
            self.inc = unet.DoubleConv_1D(n_channels, 32)#,padding_mode=padding_mode,activation='relu')
            self.down1 = unet.Down_1D(32, 64)
            self.down2 = unet.Down_1D(64, 128)
            ...
            self.up3 = unet.Up_1D(128, 64 // factor, bilinear)
            self.up4 = unet.Up_1D(64, 32, bilinear)
            self.outc = unet.OutConv_1D(32, n_classes)
    ...
    class UNet_1D(torch.nn.Module):
        def __init__(self, n_channels, n_classes, bilinear=False):
            ...
            self.inc = unet.DoubleConv_1D(n_channels, 8)#,padding_mode=padding_mode,activation='relu')
            self.down1 = unet.Down_1D(8, 16)
            self.down2 = unet.Down_1D(16, 32)
            self.down3 = unet.Down_1D(32, 64)
            factor = 2 if bilinear else 1
            
            #self.down4 = Down(512, 1024 // factor)
            #self.up1 = Up(1024, 512 // factor, bilinear)
            self.up2 = unet.Up_1D(64, 32 // factor, bilinear)
            self.up3 = unet.Up_1D(32, 16 // factor, bilinear)
            self.up4 = unet.Up_1D(16, 8, bilinear)
            self.outc = unet.OutConv_1D(8, n_classes)
...
if __name__ == '__main__':
    ...
    if flagProcess == 0: ## training model from scratch
        ...
        mod.hparams.alpha_mse_rec = 0.1 #(dT-dt_forecast)/dT #0.75
        ...
        pathCheckPOint = 'resL63/exp02-newdata/model-l63-newforecast_055--rec001--for100--init010-unet2-exp02-newdata-Noise01-igrad05_02-dgrad25-drop20-epoch=252-val_loss=2.77.ckpt'
        pathCheckPOint = 'resL63/exp02-newdata/model-l63-newforecast_055--rec001--for100--init010-unet2-exp02-newdata-Noise01-igrad05_02-dgrad25-drop20-epoch=270-val_loss=2.74.ckpt'
